        ...
        <div className="md:col-span-3">
          ...
          <div>
            ...
            <div className="flex flex-col">
              <div className="text-tech_light_color">
                <div>
                  {blog.body && (
                    <PortableText
                      value={blog.body}
...
const BlogLeft = async ({ slug }: { slug: string }) => {
  ...
  return (
    <div>
      <div className="border border-tech_light_color p-5 rounded-md">
        <Title className="text-base">Blog Categories</Title>
        <div className="space-y-2 mt-2">
          {categories?.map(({ blogcategories }, index) => (
            <div
              key={index}
              className="text-tech_light_color flex items-center justify-between text-sm font-medium"
            >
              <p>{blogcategories[0]?.title}</p>
              <p className="text-tech_dark_color font-semibold">{`(1)`}</p>
            </div>
          ))}
        </div>
      </div>
      <div className="border border-tech_light_color p-5 rounded-md mt-10">
            ...
            >
              {blog?.mainImage && (
                <Image
                  ...
                  width={100}
                  height={100}
                  className="w-16 h-16 rounded-full object-cover border-[1px] border-tech_orange/10 group-hover:border-tech_orange hoverEffect"
                />
              )}
              <p className="line-clamp-2 text-sm text-tech_light_color group-hover:text-tech_orange hoverEffect">
                {blog?.title}
              </p>
            </Link>
          ))}
